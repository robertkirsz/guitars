$(function () {
  ...
  var gryf = $('#gryf'),
    ...
    rozszerzenieDzwieku = Modernizr.audio.m4a
      ? '.m4a'
      : Modernizr.audio.ogg
      ? '.ogg'
      : '.mp3', //Rozszerzenie pliku audio
  ...
  $('#panel-nagrywania button, #tabulatura button, #panel-dolny button').each(
    function () {
      $(this)
  ...
  if (localStorage['gitara'] != null) {
    ...
    if (aktywnaGitara == 'strat') {
      gryf.css(
        'background-size',
        '21px 21px, 21px 21px, 21px 21px, 21px 21px, 21px 21px, 21px 21px, 1309px 240px'
      )
    } else if (aktywnaGitara == 'acoustic') {
      gryf.css(
        'background-size',
        szerokoscProgu * 0.613 +
          'px 132px, ' +
          szerokoscProgu * 0.613 +
          'px 132px, ' +
          szerokoscProgu * 0.613 +
          'px 132px, ' +
          szerokoscProgu * 0.508 +
          'px 147px, ' +
          szerokoscProgu * 0.524 +
          'px 150px, ' +
          szerokoscProgu * 0.451 +
          'px 165px, 848px 240px'
      )
    } else {
      gryf.css(
        'background-size',
  ...
  gitara.on('click', function () {
    ...
    if (opera) {
      localStorage.setItem('gitara', aktywnaGitara)
    }
  })
  ...
  gitara.on('click', '.podpis', function (e) {
    ...
    $('#ciemne-tlo .' + $(this).attr('class').split(' ')[1])
      .addClass('active')
      .siblings('.artysta')
      .removeClass('active')
  ...
  ciemneTlo.on('click', '.pomoc, .artysta', function (e) {
    e.stopPropagation()
  })
  ...
  var pomoc = {
    ...
    inicjalizacja: function () {
      ...
      this.el.nawigacja.on('click', function () {
        pomoc.klikniecie($(this))
      })
      //Zapisz różnicę wymiaru elementów jako ograniczenie ruchu przy przesuwaniu obrazka
      this.el.ograniczenieLeft =
        $('#ciemne-tlo .obrazek').width() - $('#ciemne-tlo .grafika').width()
      this.el.ograniczenieTop =
        $('#ciemne-tlo .obrazek').height() - $('#ciemne-tlo .grafika').height()
      //Podświetlanie wskaźników po najechaniu kursorem na instrukcję
      this.el.instrukcje.on(
        {
          mouseenter: function () {
            pomoc.ruchPodswietlenia($(this))
          },
          mouseleave: function () {
            pomoc.powrotPodswietlenia($(this))
          }
        },
        'li'
      )
    ...
    ruchPodswietlenia: function (to) {
      ...
      if (top < pomoc.el.ograniczenieTop) {
        top = pomoc.el.ograniczenieTop
      } else if (top > 0) {
        top = 0
      }
      //Ogranicza ruch w poziomie
      if (left < pomoc.el.ograniczenieLeft) {
        left = pomoc.el.ograniczenieLeft
      } else if (left > 0) {
        left = 0
      }
  ...
  var lupa = {
    ...
    inicjalizacja: function (element) {
      ...
      if (!Modernizr.pointerevents) {
        element.prepend($('<div />', { class: 'ekran' }))
      }
      element.on({
        mouseenter: function () {
          var $this = $(this)
        ...
        mousemove: function (e) {
          ...
          var lupaX = lupa.kursor.X - lupa.ramka.zdjecie - lupa.rozmiar.szer,
            ...
            tloX =
              Math.round(
                (lupa.kursor.X * lupa.duzy.szer) / lupa.maly.szer -
                  lupa.rozmiar.szer
              ) + lupa.ramka.lupa,
            tloY =
              Math.round(
                (lupa.kursor.Y * lupa.duzy.wys) / lupa.maly.wys -
                  lupa.rozmiar.wys
              ) + lupa.ramka.lupa,
            tlo = -tloX + 'px ' + -tloY + 'px'
          //Aktualizuj lupę
  ...
  var galeria = {
    ...
    inicjalizacja: function (szerokosc, czas, przesuniecie) {
      ...
      this.el.slajdy
        .css('background-size', galeria.szerokoscSlajdu + 'px')
        .add(this.el.okienko)
        .css('width', galeria.szerokoscSlajdu + 'px')
      ...
      this.el.galeria.each(function () {
        ...
        $this
          .find('.pasek')
          .width(galeria.szerokoscSlajdu * $this.find('.slajd').size())
    ...
    klikniecie: function (to) {
      ...
      if (!to.is('.active')) {
        this.ustalWysokosc(
          to.parent().siblings('.okienko').find('.slajd').eq(to.index())
        )
    ...
    ustalWysokosc: function (slajd) {
      ...
      if (!slajd.data('wysokosc')) {
        ...
        img.src = slajd.css('background-image').replace(/url\(|\)$|"/gi, '')
        //Po załadowaniu obrazka
        $(img).on('load', function () {
          //Pobierz jego wysokość i nadaj ją elementowi ze slajdami
          var wysokosc =
            (parseInt(slajd.css('background-size'), 10) / img.width) *
            img.height
    ...
    przewijanie: function () {
      ...
      if (galeria.przesuniecie) {
        var przesuwanie =
            ((pozScrl - szerSljd * lewy) * galeria.przesuniecie) / szerSljd, //Wraz z ruchem scrollbara zmienia wartość od 0 do 'maxPrzesuniecie' dla każdego slajdu od nowa
      ...
      if (lewy == prawy) {
        ...
        slajdy
          .eq(lewy)
          .css({ 'background-position': +przesuwanie * wzmocnienie + 'px 0' })
        slajdy
          .eq(prawy)
          .css({
            'background-position':
              +(przesuwanie - przesuniecie) * wzmocnienie + 'px 0'
          })
      }
    },
    autoPrzewijanie: function (podpis) {
      interwalFunkcja = setInterval(function () {
        galeria.interwal(podpis)
      }, galeria.czasInterwalu)
    },
    interwal: function (artysta) {
      ...
      $this
        .find('a')
        .eq(galeria.zmienneInterwalu[artysta])
        .addClass('active')
        .siblings('a')
        .removeClass('active')
      //Ustal wysokość slajdu
      galeria.ustalWysokosc(
        $this.find('.slajd').eq(galeria.zmienneInterwalu[artysta])
      )
      //Przesuń slajdy
      $this.find('.okienko').animate(
        {
          scrollLeft:
            galeria.zmienneInterwalu[artysta] * galeria.szerokoscSlajdu
        },
        1000
      )
      ...
      if (galeria.zmienneInterwalu[artysta] > $this.find('.slajd').size() - 1) {
        galeria.zmienneInterwalu[artysta] = 0
      }
    },
  ...
  $(window).width() > 1366
    ? galeria.inicjalizacja(500, 5000)
    : galeria.inicjalizacja(400, 5000)

  // ---------- DŹWIĘKI -----------------

  //Zapełnij tablicę plikami Audio i dodaj do nich ścieżki do pliku
  for (var i = 0; i < 6; i++) {
    zestawDzwiekow['acoustic'].push(
      new Audio('dzwieki/acoustic' + rozszerzenieDzwieku)
    )
    zestawDzwiekow['les-paul'].push(
      new Audio('dzwieki/les-paul' + rozszerzenieDzwieku)
    )
    zestawDzwiekow['strat'].push(
      new Audio('dzwieki/strat' + rozszerzenieDzwieku)
    )
  }
  ...
  var odegrajZapisane = function (
    ...
    if (deaktywacja) {
      $('.usunOstatnie, .usunPamiec').attr('disabled', 'disabled')
    }
    ...
    var petlaOdgrywania = function () {
      ...
      if (odgrywanie) {
        //Wyświetl informację
        informacja(
          'Odgrywam dzwiek ' + (numerDzwieku + 1) + ' z ' + struny.length
        )
        ...
        //Zaznaczaj progi
        $('tr')
          .eq(struny[numerDzwieku])
          .find('td')
          .eq(progi[numerDzwieku])
          .addClass('active')
        aktualizujWskazniki()
        ...
        if (numerDzwieku < struny.length - 1) {
          ...
          setTimeout(function () {
            ...
            if (odgrywanie) {
              informacja('Odtwarzanie zakonczone')
            }
            //Wyłącz odgrywanie
            odgrywanie = false
            gryf.find('td').removeClass('active')
            ...
            //Aktywuj przyciski
            if (deaktywacja) {
              $('.usunOstatnie, .usunPamiec').removeAttr('disabled')
            }
  ...
  var bazaPowerChordow = {
      akordA: $(
        '#struna3 td:nth-child(3), #struna4 td:nth-child(3), #struna5 td:nth-child(1)'
      ),
      akordB: $(
        '#struna3 td:nth-child(5), #struna4 td:nth-child(5), #struna5 td:nth-child(3)'
      ),
      akordC: $(
        '#struna3 td:nth-child(6), #struna4 td:nth-child(6), #struna5 td:nth-child(4)'
      ),
      akordD: $(
        '#struna2 td:nth-child(4), #struna3 td:nth-child(3), #struna4 td:nth-child(1)'
      ),
      akordE: $(
        '#struna4 td:nth-child(3), #struna5 td:nth-child(3), #struna6 td:nth-child(1)'
      ),
      akordF: $(
        '#struna4 td:nth-child(4), #struna5 td:nth-child(4), #struna6 td:nth-child(2)'
      ),
      akordG: $(
        '#struna4 td:nth-child(6), #struna5 td:nth-child(6), #struna6 td:nth-child(4)'
      )
    },
    bazaAkordowKlawiatura = {
      'A Dur': $(
        '#struna1 td:eq(0), #struna2 td:eq(2), #struna3 td:eq(2), #struna4 td:eq(2), #struna5 td:eq(0)'
      ),
      'B Dur': $(
        '#struna1 td:eq(2), #struna2 td:eq(4), #struna3 td:eq(4), #struna4 td:eq(4), #struna5 td:eq(2)'
      ),
      'C Dur': $(
        '#struna1 td:eq(0), #struna2 td:eq(1), #struna3 td:eq(0), #struna4 td:eq(2), #struna5 td:eq(3)'
      ),
      'D Dur': $(
        '#struna1 td:eq(2), #struna2 td:eq(3), #struna3 td:eq(2), #struna4 td:eq(0)'
      ),
      'E Dur': $(
        '#struna1 td:eq(0), #struna2 td:eq(0), #struna3 td:eq(1), #struna4 td:eq(2), #struna5 td:eq(2), #struna6 td:eq(0)'
      ),
      'F Dur': $(
        '#struna1 td:eq(1), #struna2 td:eq(1), #struna3 td:eq(2), #struna4 td:eq(3), #struna5 td:eq(3), #struna6 td:eq(1)'
      ),
      'G Dur': $(
        '#struna1 td:eq(3), #struna2 td:eq(0), #struna3 td:eq(0), #struna4 td:eq(0), #struna5 td:eq(2), #struna6 td:eq(3)'
      )
    }
  ...
  suwak.slider({
    ...
    start: function (event, ui) {
      glosnosc.addClass('active')
    },
    stop: function (event, ui) {
      glosnosc.removeClass('active')
      //Zapisywanie stanu aplikacji dla Opery
      if (opera) {
        localStorage.setItem('glosnosc', ui.value)
      }
    }
  })
      ...
      $('<li />', { class: 'nagranie' })
        .append(
          $('<div />', {
            ...
            .append($('<p />', { class: 'tytul', text: tytuł }))
        )
        .data('numer', indeks)
        .hide()
        .fadeIn()
  ...
  var sprawdzNagrania = function () {
    $('.nagranie').size() > 10
      ? $('#nagrania').addClass('overflow')
      : $('#nagrania').removeClass('overflow')
  }
  sprawdzNagrania()
  ...
  $('.usunOstatnie').on('click', function () {
    ...
    if (zapisaneStruny.length < 1) {
      ...
      zapisaneProgi.pop()
      informacja(
        'Usunieto ostatni zapis<br>Długosc nagrywania: ' + zapisaneStruny.length
      )
    }
  })
  ...
  $('#nagrania').on('click', 'li:not([disabled="disabled"])', function () {
    ...
    if (!usuwanieElementow) {
      ...
      var indeks = bazaNagran
        .map(function (el) {
          return el.indeks
        })
        .indexOf($this.data('numer'))
      ...
      if ($this.hasClass('stop')) {
        ...
        odegrajZapisane(
          bazaNagran[indeks].struny,
          bazaNagran[indeks].początek,
          bazaNagran[indeks].progi,
          bazaNagran[indeks].odstępy,
          $this,
          false,
          function () {
            //Przywróc elementom neutralny wygląd i aktywuj przycisk play
  ...
  if (localStorage['zapisaneAkordy'] != null) {
    ...
    //Jeśli nie, ustaw je jak poniżej
    var bazaAkordow = {
      'A Dur': [0, 2, 2, 2, 0, -1],
      'B Dur': [2, 4, 4, 4, 2, -1],
      'C Dur': [0, 1, 0, 2, 3, -1],
    ...
    switch ($(this).attr('class')) {
      case 'dodaj': //Jeśli kliknięto "Zapisz"...
        tabulatura.find('.aktualna-pozycja').each(function (i) {
          ...
          $(this)
            .clone()
            .removeClass('aktualna-pozycja')
            .addClass('pozycja')
            .appendTo($('.znaki')[i])
        })
        //Wyprostuj element, jeśli jego szerokość jest większa, niż 400px
        if (tabulatura.width() > 400) {
          tabulatura.removeClass('obrot')
        }
        break
      ...
      case 'drukuj': //Jeśli kliknięto "Otwórz do druku"...
        ...
        $(window.open().document.body).html(
          '<style>body { font-family: monospace; list-style: none; }</style>' +
            tabulatura.html()
        )
    }
  })
  ...
  $('#akordy, #nagrania, #tabulatura').prepend(
    $(
      '<button class="minimalizuj"><span class="znak">-</span><span class="tekst">Minimalizuj</span></button>'
    )
  )
  ...
  $('.minimalizuj').on('click', function () {
    ...
    $this.parent().fadeOut('normal', function () {
      ...
      if (opera) {
        if ($('#akordy').is(':hidden')) {
          localStorage.setItem('akordy', 'hidden')
        } else {
          localStorage.setItem('akordy', 'visible')
        }
        if ($('#nagrania').is(':hidden')) {
          localStorage.setItem('nagrania', 'hidden')
        } else {
          localStorage.setItem('nagrania', 'visible')
        }
        if ($('#tabulatura').is(':hidden')) {
          localStorage.setItem('tabulatura', 'hidden')
        } else {
          localStorage.setItem('tabulatura', 'visible')
        }
      }
    })
    //Umieść w panelu dolnym odpowiednią ikonkę zminimalizowanego okna
    $('#panel-dolny')
      .find('.' + $this.parent().attr('id'))
      .fadeIn()
  })
  ...
  $('.oznaczenia').on('click', function () {
    ...
    //Zapisywanie stanu aplikacji dla Opery
    if (opera) {
      if ($('.oznaczenia').hasClass('active')) {
        localStorage.setItem('ukryte', 'tak')
      } else {
        localStorage.setItem('ukryte', 'nie')
      }
    }
  })
  ...
  $('.wyciszGryf').on('click', function () {
    ...
    //Zapisywanie stanu aplikacji dla Opery
    if (opera) {
      if ($('.wyciszGryf').hasClass('active')) {
        localStorage.setItem('wyciszenie', 'tak')
      } else {
        localStorage.setItem('wyciszenie', 'nie')
      }
    }
  })
  ...
  $('#panel-dolny .pomoc').on('click', function () {
    ...
    if ($('.podswietlenie').height() == 0) {
      ...
      $('.podswietlenie')
        .height($('.pomoc nav ul li').outerHeight())
